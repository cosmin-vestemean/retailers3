...
ALTER TABLE CCCSFTP ADD EDIPROVIDER INT NULL;
INSERT INTO CCCSFTP (
        PORT,
        TRDR_RETAILER,
        URL,
...
ALTER TABLE CCCXMLS1MAPPINGS ADD
    ...
    POSITION INT NULL,
    IS_ATTRIBUTE TINYINT NOT NULL DEFAULT 0,    PREFIX_FILTER VARCHAR(100) NULL,
...
SET XML_PATH = XMLNODE,
    ...
    TRANSFORMATION = CASE 
                      ...
                      ELSE NULL
                    END,    TRANSFORMATION_PARAMS = CASE
                              WHEN SQL IS NOT NULL AND SQL <> '' THEN SQL
                              ELSE NULL
                            END;

-- Ensure all existing field mappings are set to active
UPDATE CCCXMLS1MAPPINGS SET ACTIVE = 1 WHERE ACTIVE IS NULL OR ACTIVE = 0;
...
INSERT INTO CCCCONNTYPE (NAME) VALUES 
('SFTP'),
('API'),
('FTP'),
('WebService');

-- Insert sample EDI providers
INSERT INTO CCCEDIPROVIDER (NAME, CONNTYPE) VALUES 
('DocXchange', 1), -- SFTP
('Direct API', 2),  -- API
('Legacy FTP', 3);  -- FTP

-- Add foreign key constraint for EDIPROVIDER in CCCSFTP
ALTER TABLE CCCSFTP ADD CONSTRAINT FK_CCCSFTP_EDIPROVIDER 
FOREIGN KEY (EDIPROVIDER) REFERENCES CCCEDIPROVIDER(CCCEDIPROVIDER);

-- Update existing CCCSFTP records to link with a default provider (DocXchange)
UPDATE CCCSFTP SET EDIPROVIDER = 1 WHERE EDIPROVIDER IS NULL;

CREATE TABLE CCCEDIUSERS (
    CCCEDIUSERS INT NOT NULL IDENTITY(1,1) PRIMARY KEY,
    EMAIL VARCHAR(255) NOT NULL UNIQUE,
    PASSWORD_HASH VARCHAR(255) NOT NULL,
    ROLE VARCHAR(20) NOT NULL, -- 'ADMIN', 'CLIENT'
    TRDR_CLIENT INT NULL, -- NULL for admins, specific client for client users
    ACTIVE TINYINT NOT NULL DEFAULT 1,
...
-- Add foreign key constraint for EDIPROVIDER
ALTER TABLE CCCSFTP ADD CONSTRAINT FK_CCCSFTP_EDIPROVIDER 
FOREIGN KEY (EDIPROVIDER) REFERENCES CCCEDIPROVIDER(CCCEDIPROVIDER);

-- Insert some sample connection types
INSERT INTO CCCCONNTYPE (NAME) VALUES 
('SFTP'),
('API'),
('FTP'),
...
-- Update existing CCCSFTP records to link with a default provider
UPDATE CCCSFTP SET EDIPROVIDER = 1 WHERE EDIPROVIDER IS NULL; -- Link to DocXchange